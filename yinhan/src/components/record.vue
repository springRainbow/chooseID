<template>
	<div id="record">
		...
		<div class="main">
			<button @click="index=0,bol=false" :class="index==0?'active':''">我心动的</button>
			<button @click="index=1,bol=true" :class="index==1?'active':''">对我心动</button>
		</div>
		<div class="select">
...
<script>
	export default {
		...
		methods: {
			...
			statusChange(idx) {
				this.params.status = idx;
				console.log(this.params.status)
				this.commonAxios();
			},
			timeChange(idx) {
				this.params.daterange = idx;
...
<style>
	...
	#record .figuretwo p:nth-of-type(1) {
		font-size: 1.07rem;
	}
	
	#record .figuretwo p:nth-of-type(1) span:nth-of-type(2) {
		margin-left: 1.2rem;
		color: #B674FD;
	}
	
	#record .figuretwo p:nth-of-type(2) {
		font-size: 1rem;
		color: #666666;
	}
	
	#record .figurethree button {
		width: 4.86rem;
		height: 2.57rem;
		background: #B674FD;
